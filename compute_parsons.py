from code_on_shs100k.load_features import split_training_data, load_parsons
from code_on_shs100k.simple_model import parsons_model
...
if __name__ == '__main__':
    ...
    print("Welcome to the Parsons Code training program")
    ...
    data = load_parsons(audio_dir="/research/d2/fyp23/tcor0/train_melody01",extracted_cqt = True,all_data_saved_path = "C:\\1112parsons\\parsons.npy")
    #data = load_parsons(audio_dir="F:/FYP/tim_data/train_melody01",extracted_cqt = True,all_data_saved_path = "")
    ...
    model = parsons_model(1,number_of_class)
    #multi-gpu usage, still investigating
    #print(summary(model, (1999,), batch_size=batch_size))  # 128 or 256 when in server
    ...
    if torch.cuda.is_available():
        ...
        print(summary(model, (1,1999), batch_size=batch_size)) #1999 for size 2000 [batch_size, channels, sequence_length]
    #load train and test data with the Data Class and dataloader function
    traindata = Data(x_train,y_train)
    #traindata = Data(x_train, one_hot_y_train)
    trainloader = DataLoader(traindata, batch_size=batch_size,shuffle=True, num_workers=1)

    testdata = Data(x_test, y_test)
    #testdata = Data(x_test,one_hot_y_test)
    testloader = DataLoader(testdata, batch_size=batch_size,shuffle=True, num_workers=1)
    ...
    for epoch in range(epochs):
        loop = tqdm(trainloader) #,disable=True
        running_loss = 0.0
